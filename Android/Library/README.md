...
## [RxJava Backpressure(배압)](https://reactivex.io/documentation/operators/backpressure.html)

> RxJava의 배압은 관찰 가능한 소스가 다운스트림이 소비할 수 있는 것보다 빠른 속도로 데이터를 방출하는 상황을 처리하는 데 사용되는 메커니즘을 나타냅니다. 소비자가 데이터 흐름을 제어할 수 있도록 함으로써 소비자가 압도당하는 것을 방지하는 데 도움이 됩니다.

- 배압은 반응형 프로그래밍에서 중요한 개념이며 관찰 가능한 소스가 소비자가 처리할 수 있는 것보다 더 빠르게 데이터를 생성하는 시나리오를 다룹니다. RxJava에서 배압 메커니즘은 생산자와 소비자 사이의 균형을 보장하면서 방출된 데이터를 느리게 하거나 버퍼링하도록 소비자가 생산자에게 신호를 보낼 수 있는 방법을 제공합니다.

- 기본적으로 RxJava는 소비자의 무제한 요구를 가정합니다. 즉, 생산자가 원하는 만큼 빠르게 데이터를 내보낼 수 있습니다. 그러나 이로 인해 소비자가 동일한 속도로 데이터를 처리할 수 없을 때 문제가 발생하여 메모리에 데이터가 축적되거나 시스템 오류가 발생할 수 있습니다.

배압을 처리하기 위해 RxJava는 다음과 같은 몇 가지 전략을 제공합니다.

- **Missing**: 이것은 소비자가 자신의 속도로 데이터를 처리한다고 가정하는 기본 전략입니다. 소비자가 따라잡지 못하면 데이터 손실 또는 메모리 문제가 발생할 수 있습니다.
- **Buffer**: 이 전략에서는 중간 버퍼를 사용하여 소비자가 항목을 처리할 준비가 될 때까지 방출된 항목을 저장합니다. 버퍼의 크기는 제한되어 있으며 가득 차면 공간이 확보될 때까지 속도를 늦추거나 중지하라는 지시를 받습니다.
- **Drop**: 이 전략을 사용하면 버퍼가 가득 차면 생산자가 소비자가 따라갈 수 없는 방출된 항목을 삭제하기 시작합니다. 이전 데이터보다 최신 데이터를 우선시합니다.
- **Latest**: 이 전략은 소비자가 처리할 수 없는 방출된 항목을 버리고 가장 최근에 방출된 항목만 유지합니다. 소비자가 가장 최근의 데이터를 받도록 합니다.
- **Error**: 이 전략에서는 소비자가 생산자를 따라갈 수 없을 때 배압 위반을 나타내는 예외가 발생합니다.

생산자와 소비자 간의 균형을 유지하고 리소스 고갈 또는 데이터 손실을 방지하려면 애플리케이션의 특정 요구 사항 및 특성을 기반으로 적절한 배압 전략을 선택하는 것이 중요합니다.
